"""Epic edition runner — common patches + epic build package overlay."""
...
def _apply_build_package(root: Path, execute: bool) -> dict:
    ...
    build_url = os.environ.get("EPIC_BUILD_URL", "").strip()

    if not build_dir.is_dir() and not build_url:
        return {"status": "SKIPPED_TEMPORARY", "reason": "build/ missing and EPIC_BUILD_URL not set"}
...
def run_edition(
    ...
    if build.get("status") == "FAILED":
        errors.extend(build.get("errors", ["epic build_package FAILED"]))
