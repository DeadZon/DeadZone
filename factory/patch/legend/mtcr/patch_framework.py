"""
Legend MTCR patch: framework.jar  <-  Framework_Legend.mtcr

Target:  system/framework/framework.jar
MTCR:    Framework_Legend.mtcr
Add-DEX: freamwork add.dex  (typo preserved from source filename)

Exposes:
  apply_framework_mtcr_patch(work_dir, report, execute) -> dict
"""
...
def apply_framework_mtcr_patch(work_dir: Path, report: dict, execute: bool) -> dict:
    """
    Apply Framework_Legend.mtcr to the already-unpacked framework.jar workspace.

    Parameters
    ----------
    work_dir : Path
        Directory containing smali_classes* dirs for framework.jar.
    report : dict
