"""
Legend MTCR patch: services.jar  <-  Service_Legend.mtcr

Target:  system/framework/services.jar
MTCR:    Service_Legend.mtcr

Exposes:
  apply_services_mtcr_patch(work_dir, report, execute) -> dict
"""
...
def apply_services_mtcr_patch(work_dir: Path, report: dict, execute: bool) -> dict:
    """
    Apply Service_Legend.mtcr to the already-unpacked services.jar workspace.

    Parameters
    ----------
    work_dir : Path
        Directory containing smali_classes* dirs for services.jar.
    report : dict
