...
Pipeline order (execute mode):
  ...
  5.  Cross-JAR stages (operate on the shared work_dir smali roots):
        e. Signature bypass (if factory.patch.mods.legend.signature_bypass_legacy present).
        f. JAR misc legacy (if factory.patch.mods.legend.jar_misc_legacy present).
        g. Kaori legacy (if factory.patch.mods.legend.kaori_legacy present).
...
CLI:
  # dry-run:
  python -m factory.patch.mods.legend.jar_patch \\
...
from factory.patch.mods.legend.jar_rules import (
    LEGEND_JAR_RULES,
    LegendJarRule,
    find_mtcr_dir,
    resolve_mtcr_path,
)
from factory.patch.mods.legend.add_dex_rules import find_add_dex_for_jar
...
try:
    from factory.patch.mods.legend.archived_legacy.signature_bypass_legacy import (
...
try:
    from factory.patch.mods.legend.archived_legacy.jar_misc_legacy import (
...
try:
    from factory.patch.mods.legend.archived_legacy.kaori_legacy import (
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m factory.patch.mods.legend.jar_patch",
