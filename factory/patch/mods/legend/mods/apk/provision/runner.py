...
from factory.patch.mods.legend.mods.apk.provision.branding import apply_visible_branding
from factory.patch.mods.legend.mods.apk.provision.model import load_class_patch
...
def _load_smali_rules() -> list:
    result = []
    pkg = "factory.patch.mods.legend.mods.apk.provision.smali"
...
def _apply_xml_rules(decompiled_dir: Path, dry_run: bool) -> dict:
    rules = []
    rules.extend(_load_list("factory.patch.mods.legend.mods.apk.provision.manifest.manifest_rules", "MANIFEST_RULES"))
    rules.extend(_load_list("factory.patch.mods.legend.mods.apk.provision.manifest.xml_rules", "XML_RULES"))
    results = []
    for rule in rules:
...
def _apply_resource_rules(decompiled_dir: Path, dry_run: bool) -> dict:
    rules = []
    rules.extend(_load_list("factory.patch.mods.legend.mods.apk.provision.resources.arsc_rules", "ARSC_RULES"))
    rules.extend(_load_list("factory.patch.mods.legend.mods.apk.provision.resources.values_rules", "VALUES_RULES"))
    results = []
    for rule in rules:
...
def _apply_branding_rules(decompiled_dir: Path, dry_run: bool) -> dict:
    explicit = _load_list("factory.patch.mods.legend.mods.apk.provision.resources.branding_string_rules", "BRANDING_STRING_RULES")
    changes = []
    review = []
    unsafe = []
...
def apply_legend_provision_patch(project_dir: Path, flavor: str, execute: bool = False, work_dir: Path | None = None) -> dict:
    ...
    static_xml_count = len(_load_list("factory.patch.mods.legend.mods.apk.provision.manifest.manifest_rules", "MANIFEST_RULES")) + len(_load_list("factory.patch.mods.legend.mods.apk.provision.manifest.xml_rules", "XML_RULES"))
    static_res_count = len(_load_list("factory.patch.mods.legend.mods.apk.provision.resources.arsc_rules", "ARSC_RULES")) + len(_load_list("factory.patch.mods.legend.mods.apk.provision.resources.values_rules", "VALUES_RULES"))
    report.update({
    ...
    if not execute:
        ...
        xml = {"count": len(_load_list("factory.patch.mods.legend.mods.apk.provision.manifest.manifest_rules", "MANIFEST_RULES")) + len(_load_list("factory.patch.mods.legend.mods.apk.provision.manifest.xml_rules", "XML_RULES"))}
        res = {"count": len(_load_list("factory.patch.mods.legend.mods.apk.provision.resources.arsc_rules", "ARSC_RULES")) + len(_load_list("factory.patch.mods.legend.mods.apk.provision.resources.values_rules", "VALUES_RULES"))}
        report.update({
