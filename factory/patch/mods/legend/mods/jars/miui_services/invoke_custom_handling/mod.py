"""
Legend JAR mod — miui_services/invoke_custom_handling

Scans miui-services.jar smali files for invoke-custom instructions and applies
...
METADATA = {
    ...
    "target_classes":     ["(any *.smali in miui-services unpack_dir with invoke-custom)"],
...
def apply(unpack_dir: Path, config: dict, dry_run: bool = True) -> dict[str, Any]:
    ...
    if errors:
        ...
        report["status"] = "SKIPPED"
        report["warnings"].append("No smali files with invoke-custom found in miui-services unpack_dir.")
    elif dry_run:
