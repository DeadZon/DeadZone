"""
Legend JAR mod — miui_framework/gboard_replace_baidu

Replaces the Baidu IME package name with Gboard in miui-framework.jar smali files.
...
from factory.patch.mods.legend.mods._shared import gboard_replace_in_dir
...
def apply(unpack_dir: Path, config: dict, dry_run: bool = True) -> dict[str, Any]:
    ...
    not_found = [fn for d in details for fn in [d.get("file", "")] if d["status"] == "MISSING"]
    ...
    if errors:
        ...
        report["status"] = "SKIPPED"
        report["warnings"].append("All target files missing from miui-framework unpack_dir.")
    elif not patched:
        report["status"] = "SKIPPED"
