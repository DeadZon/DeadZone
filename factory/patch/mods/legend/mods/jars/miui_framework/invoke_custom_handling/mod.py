"""
Legend JAR mod — miui_framework/invoke_custom_handling

Scans miui-framework.jar smali files for invoke-custom instructions and applies
...
from factory.patch.mods.legend.mods._shared import invoke_custom_scan_dir
...
METADATA = {
    ...
    "target_classes":     ["(any *.smali in miui-framework unpack_dir with invoke-custom)"],
...
def apply(unpack_dir: Path, config: dict, dry_run: bool = True) -> dict[str, Any]:
    ...
    if errors:
        ...
        report["status"] = "SKIPPED"
        report["warnings"].append("No smali files with invoke-custom found in miui-framework unpack_dir.")
    elif dry_run:
