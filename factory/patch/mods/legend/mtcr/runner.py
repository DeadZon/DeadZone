...
CLI:
  python -m factory.patch.mods.legend.mtcr.runner \\
...
from factory.patch.mods.legend.mtcr.smart_smali_patcher import (
    ClassMatchStatus,
    MethodMatchStatus,
    PatchApplyStatus,
    apply_smart_patch,
...
from factory.patch.mods.legend.mods.registry import (
    LEGEND_JAR_MODS,
    merge_profile,
)
...
_LEGEND_HOME    = Path(__file__).resolve().parents[1]
...
def _discover_class_modules(pkg_subdir: str) -> list[Any]:
    ...
    factory.patch.mods.legend.mods.jars.<pkg_subdir> and return them.
    """
    base_pkg = f"factory.patch.mods.legend.mods.jars.{pkg_subdir}"
...
def _run_jar_mods(
    ...
    base_pkg = f"factory.patch.mods.legend.mods.jars.{jar_group}"
