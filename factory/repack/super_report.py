...
def format_super_build_legacy_report(report: dict) -> str:
    ...
    lines = [
        ...
        f"Orig sizes source:    {report.get('original_partition_sizes_source', '(not set)')}",
        f"Manifest sizes avail: {report.get('manifest_partition_sizes_available', False)}",
        f"Manifest sizes used:  {report.get('manifest_partition_sizes_used') or '(none)'}",
        f"Manifest sizes miss:  {report.get('manifest_partition_sizes_missing') or '(none)'}",
        f"Img size fallback:    {report.get('forbidden_image_size_fallback_used', False)}",
