...
# ==========================================
# DeadZone MTK Cloud Builder (Fly.io)
# ==========================================

export SOC="mtk"
export DZ_FACTORY_NAME="DeadZone MTK Factory"
...
python _dz_tg.py send STARTING "Preparing MTK runner on 400GB Cloud" || true
...
if [ "$DZ_MODE" == "dry_run" ]; then
  python _dz_tg.py edit RUNNING_PIPELINE "Running MTK dry-run pipeline" || true
  ...
  python _dz_tg.py edit RUNNING_PIPELINE "Running MTK execute pipeline" || true
  python -m factory.pipeline.legacy_build_orchestrator \
    --rom "$ROM_FILE" --output-dir output --build-name "$FINAL_NAME" \
    --device "$EFFECTIVE_DEVICE" --soc "$SOC" --platform "$IN_PLATFORM" \
    --flavor "$FLAVOR" --android-version "$IN_ANDROID_VERSION" \
